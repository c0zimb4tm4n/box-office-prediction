...
file_path = os.path.join(script_directory, '../data/cleaned/data_clean_v6.csv')
...
with tab3:
   ...
   film_data['Revenue_InflationCorrected'] = pd.to_numeric(film_data['Revenue_InflationCorrected'])
   revenue_by_genre = film_data.groupby('genres')['Revenue_InflationCorrected'].sum().sort_values(ascending=False)
...
with tab4:
   fig, ax = plt.subplots()
   ax.scatter(film_data['averageRating'], film_data['Revenue_InflationCorrected'], alpha=0.5)
...
with tab5:
   average_revenue_by_genre = film_data.groupby('genres')['Revenue_InflationCorrected'].mean().sort_values(ascending=False)
   fig, ax = plt.subplots()
...
with tab9:
   revenue_by_actor = film_data.groupby('actor')['Revenue_InflationCorrected'].mean().sort_values(ascending=False).head(10)
   fig, ax = plt.subplots()
...
with tab10:
   revenue_by_actress = film_data.groupby('actress')['Revenue_InflationCorrected'].mean().sort_values(ascending=False).head(10)
   fig, ax = plt.subplots()
...
with tab11:
   revenue_by_director = film_data.groupby('director')['Revenue_InflationCorrected'].mean().sort_values(ascending=False).head(10)
   fig, ax = plt.subplots()
...
with tab12:
   average_rating_by_director = film_data.groupby('director')['averageRating'].mean().sort_values(ascending=False).head(10)
   fig, ax = plt.subplots()
...
with tab16:
   selected_variables = ['averageRating', 'runtimeMinutes', 'Revenue_InflationCorrected']
   pair_plot_data = film_data[selected_variables]
   pair_plot_data = pair_plot_data.rename(columns={'Revenue_InflationCorrected': 'Revenue'})
   pairplot = sns.pairplot(pair_plot_data)
   st.pyplot(pairplot.fig)

with tab17:
   selected_variables = ['averageRating', 'runtimeMinutes', 'Revenue_InflationCorrected']
   correlation_matrix_data = film_data[selected_variables]
   correlation_matrix_data = correlation_matrix_data.rename(columns={'Revenue_InflationCorrected': 'Revenue'})
...
with tab18:
   selected_variables = ['runtimeMinutes', 'averageRating', 'Revenue_InflationCorrected']
   scatter_data = film_data[selected_variables]
   scatter_data = scatter_data.rename(columns={'Revenue_InflationCorrected': 'Revenue'})

   fig = plt.figure(figsize=(10, 8))
   ax = fig.add_subplot(111, projection='3d')
   ax.scatter(scatter_data['runtimeMinutes'], scatter_data['averageRating'], scatter_data['Revenue'], c='coral', marker='o', alpha=0.5)
   ax.set_xlabel('Runtime (minutes)')
   ax.set_ylabel('Average Rating')
